...
async def room_create(system, room_info, sophia, message, message_low):
    ...
    # await sophia.send_message(message.channel, str(find_room_game) + '\n' +
    #   str(message_element) + '\n' +
    ...
    if message_element[1] in room_info.table_limits[0]:
        ...
        # await sophia.send_message(message.channel, str(room_info.minigame_session[room_count]))
        await sophia.send_message(message.channel, 'Room has successfully created.\n' +
            'Your room ID is ' + str(room_count) + '.')

    else:
        await sophia.send_message(message.channel, 'Game type is not found.' + '\n' +
            'Please format your message as ' + str(system.prefix_information) + 'roomcreate' +
            ' `Room name` `Game name` `Room Password`')


async def room_check(room_info, sophia, message, message_low):
    ...
    await sophia.send_message(message.channel, room_information)

async def room_join(room_info, sophia, message, message_low):
    ...
    if room_info.minigame_session[int(room_id)][1] is not None and \
        ...
        # await sophia.send_message(message.channel, "Password exists")
        if find_room_password is not -1:
            # await sophia.send_message(message.channel, "User enters a password")
    ...
    if is_allowed:
        if room_player_count <= roomcapacity:
            if message.author not in room_info.player_joined:
                ...
                await sophia.send_message(message.channel, 'You are now in room ' + str(room_id) + '.')

            else:
                # await sophia.send_message(message.channel, 'no')
                await sophia.send_message(message.channel, 'Unable to join since you are in a room.')
        else:
            await sophia.send_message(message.channel, 'Unable to join since the room is full.')
    else:
        await sophia.send_message(message.channel, "Invalid room ID or room password.")

# async def room_leave(sophia, message, message_low, minigame_session, player_joined):
